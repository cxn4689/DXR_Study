...
[shader("raygeneration")] void RayGen() {
  ...
  float4 all_3 = gOutput3[launchIndex];//y is used to indicate full accumulate of SAMPLE_NUM
  ...
  if(1)
  {
	  ...
	  else
	  {
		  ...
		  if (all_3.y < 1.0)
		  {
			  float3 tmp = accume_rgb * (float)SAMPLE_NUM / count;
			  tmp.x = (tmp.x > 1.0) ? 1.0 : (tmp.x < 0.0) ? 0.0 : tmp.x;
			  tmp.y = (tmp.y > 1.0) ? 1.0 : (tmp.y < 0.0) ? 0.0 : tmp.y;
			  tmp.z = (tmp.z > 1.0) ? 1.0 : (tmp.z < 0.0) ? 0.0 : tmp.z;
			  tmp.x = pow(tmp.x, 1.0 / 2.2);
		  ...
		  if (all_3.z == 2021 && all_3.y == 0)
		  {
			  gOutput[launchIndex] = float4(1, 0, 0, 0);
		  }
#endif
#endif
	  }
	  gOutput2[launchIndex] = float4(accume_rgb, count);
	  gOutput3[launchIndex] = float4(seed,all_3.y,all_3.z,count);
