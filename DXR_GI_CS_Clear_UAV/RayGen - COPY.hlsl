...
[shader("raygeneration")] void RayGen() {
  ...
  payload.random.x = gOutput[launchIndex].w + 1000000.0;// +launchIndex.x * launchIndex.y;
  ...
  //gOutput2: x,y,z RGB w = count
  float count = gOutput2[launchIndex].w;
  float3 accume_rgb = gOutput2[launchIndex].xyz;
  if (count < 0) count = 0;
  count = count + 1;
#if 0
  if (count >= ACCUME_STEP)
  {
	  ...
	  gOutput[launchIndex] = float4(tmp, payload.random.x);
	  //
	  accume_rgb = float3(0, 0, 0);
	  accume_rgb = accume_rgb + primary_color * (1.0 / ACCUME_STEP);
	  count = 1;
  }
  else
  {
  ...
  if (count == 10)
  {
	  gOutput[launchIndex].rgb = float3(1, 0, 0);
  }
  if (count == 20)
  {
	  gOutput[launchIndex].rgb = float3(0, 1, 0);
  }
  if (count == 30)
  {
	  gOutput[launchIndex].rgb = float3(0, 0, 1);
	  count = 0;
  }
#else
  //if (count == 20)
  {
	  float z = gOutput2[launchIndex].x;
	  LCGRand rng;
	  rng.state = asuint(z); //+ count;//
	  uint32_t org_state = rng.state;
	  float rand_x = lcg_randomf(rng);
	  float rand_y = lcg_randomf(rng);
	  float rand_z = lcg_randomf(rng);
	  uint32_t cur_state = rng.state;
#if 1
	  //if(rand_x != rand_y && rand_y != rand_z)
	  //if(org_state != cur_state)
	  if(z == 0)
	  {
		  gOutput[launchIndex].rgb = float3(1, 1, 0);
	  }
	  else
	  {
		  gOutput[launchIndex].rgb = float3(rand_x, rand_y, rand_z);
	  }
	  gOutput2[launchIndex].x = asfloat(rng.state);
#else
	  test_t tx;
	  tx.col = float4(1, 1, 1,1);
	  //float c2 = test1(tx);
	  gOutput[launchIndex].rgb = tx.col.rgb;
#endif
  }
#endif
#endif
