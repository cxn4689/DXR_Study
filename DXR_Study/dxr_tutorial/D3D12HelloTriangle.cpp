...
ComPtr<ID3D12RootSignature> D3D12HelloTriangle::CreatePlaneHitSignature(UINT space) {
	nv_helpers_dx12::RootSignatureGenerator rsc;
	rsc.AddRootParameter(D3D12_ROOT_PARAMETER_TYPE_SRV,0, space);
	rsc.AddRootParameter(D3D12_ROOT_PARAMETER_TYPE_SRV,1, space);//Attrib
	return rsc.Generate(m_device.Get(), true);
}
...
void D3D12HelloTriangle::CreateRaytracingPipeline() {
  ...
  m_rayGenSignature = CreateRayGenSignature(100);
  m_missSignature = CreateMissSignature(200);
  m_hitSignature = CreateHitSignature(300);
  m_hitSignature1 = CreateHitSignature(310);
  m_hitSignature2 = CreateHitSignature(320);
  m_hitSignature3 = CreateHitSignature(330);
  //
  m_PlanehitSignature = CreatePlaneHitSignature(340);
  ...
  pipeline.SetMaxPayloadSize(4 * sizeof(float) * 6); // RGB + distance
