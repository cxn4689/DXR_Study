...
StructuredBuffer<STlightPos> BLightPos : register(t3, space305);
...
[shader("closesthit")] void ClosestHit3(inout HitInfo payload,
	...
	if (payload.hit_info.y == 0.0f)//primary ray
	{
...
#endif
		{
			...
			payload.colorAndDistance = float4(payloadx.colorAndDistance.rgb * hitColor, RayTCurrent());
		...
		else
		{
			...
			float3 light_pos = BLightPos[0].pos.xyz;// LIGHT_POS;
			...
			TraceRay(
				...
				ray,
				payloadx);
			payload.random.x = payloadx.random.x;// +1280 * 720;
			...
			float cos_indrct_n = float3(0, 0, 0);
			...
			if (cur_depth < 5 && USE_INDRCT)
			{
				...
				*/
				...
				cos_indrct_n = abs(dot(dir_diffuse, norm));
				//
				HitInfo payloady;
				...
				indirect_lit = payloady.colorAndDistance.xyz; //float3(r1, r2, 0);
			...
			double cos_a_max = sqrt(1-s.rad*s.rad/(x-s.p).dot(x-s.p));
			  ...
			  e = e + f.mult(s.e*l.dot(nl)*omega)*M_1_PI;  // 1/pi for brdf
				*/
				...
				float omega = 2.0 * (1 - cos_a_max);
				direct_lit = LIGHT_E * omega * cos_n;
				//direct_lit = float3(omega*0.0, 0, 0);
			}
			//
